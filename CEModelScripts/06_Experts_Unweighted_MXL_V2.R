...
### Set core controls
apollo_control = list(
  modelName       = "Experts_Unweighted_V2",
  modelDescr      = "Experts_Unweighted_V2",
  indivID         = "ID",
  mixing = TRUE,
  nCores = 10,
  outputDirectory = "Output/Model_Unweighted"
)
...
Experts_Unweighted_V2 = apollo_estimate(apollo_beta,
                                    apollo_fixed,
                                    apollo_probabilities,
                                    apollo_inputs)

## Model output and results here alongside saving information
apollo_modelOutput(Experts_Unweighted_V2, modelOutput_settings = list(printPVal = 2))
apollo_saveOutput(Experts_Unweighted_V2, saveOutput_settings = list(printPVal = 2))
# saveRDS(Experts_Unweighted_V2, file="Experts_Unweighted_V2.rds")
...
Model <- readRDS(here("Output/Model_Unweighted",
                      "Experts_Unweighted_V2_model.rds")) ## Enter model of interest RDS here


# Conditionals: *****************************
Experts_Unweighted_V2_WTP <-
  apollo_conditionals(Model, apollo_probabilities, apollo_inputs)
Experts_Unweighted_V2_WTP %>%
  data.frame() %>%
  fwrite(sep = ",",
         here("Output/Model_Unweighted",
              "Experts_Unweighted_V2_WTP.csv"))


# Unconditionals: *****************************
# Experts_Unweighted_V2_UCWTP <-
#   apollo_unconditionals(Model, apollo_probabilities, apollo_inputs)
# Experts_Unweighted_V2_UCWTP %>%
#   data.frame() %>%
#   fwrite(sep = ",",
#          here("Output/Model_Unweighted",
#               "Experts_Unweighted_V2_UCWTP.csv"))
