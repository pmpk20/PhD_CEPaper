...
## "Table 4. The χ^2test statistic (P value in parentheses) against the
# null hypothesis that the frequency of respondents reporting each
# level of certainty was not statistically different between each level of
...
Table4 %>% write.csv(quote = FALSE, row.names = FALSE)


## Export:
Table4 %>%
  fwrite(sep=",",
         here("Output/Tables","Table4_Certainty_ChiSquare.txt"),
         row.names = TRUE,
         quote = FALSE)
