...
for (Row in 1:5){
  for (V in Variables){
    for (Col in 1:5){
      ...
      Lists_MWs[[Col]] <- paste0(Test$statistic %>% round(2),
                                 " (",
...
colnames(Table6) <- c("Variable", 1, 2, 3, 4, 5)

Table6 <- Table6 %>% arrange(Variable)


## Export results to screen for copy and pasting
Table6 %>% write.csv(quote = FALSE, row.names = FALSE)


## Export:
Table6 %>%
  fwrite(sep=",",
         here("Output/Tables","Table6_MWTestByConfidence.txt"),
         row.names = TRUE,
         quote = FALSE)
