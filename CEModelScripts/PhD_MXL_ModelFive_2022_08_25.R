#### PhDP1: EB ####
## Function: Model Five: Categorical experts and unweighted
...
### Set core controls
apollo_control = list(
  modelName       = "PhD_MXL_ModelFive_2022_08_25",
  modelDescr      = "PhD_MXL_ModelFive_2022_08_25",
  indivID         = "ID",
  mixing=TRUE,
  nCores=10
)
...
PhD_MXL_ModelFive_2022_08_25 = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)

## Model output and results here alongside saving information
apollo_modelOutput(PhD_MXL_ModelFive_2022_08_25,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(PhD_MXL_ModelFive_2022_08_25,saveOutput_settings = list(printPVal=TRUE))
# saveRDS(PhD_MXL_ModelFive_2022_08_25, file="PhD_MXL_ModelFive_2022_08_25.rds")
...
Model <- readRDS("PhD_MXL_ModelFive_2022_08_25_model.rds") ## Enter model of interest RDS here
PhD_MXL_ModelFive_2022_08_25_WTP <- apollo_conditionals(Model,apollo_probabilities,apollo_inputs )
write.csv(PhD_MXL_ModelFive_2022_08_25_WTP,"PhD_MXL_ModelFive_2022_08_25_WTP.csv")

PhD_MXL_ModelFive_2022_08_25_WTP <- data.frame(read.csv("PhD_MXL_ModelFive_2022_08_25_WTP.csv"))


# Trim the stupidly large price estimates
PhD_MXL_ModelFive_2022_08_25_WTP <- PhD_MXL_ModelFive_2022_08_25_WTP[PhD_MXL_ModelFive_2022_08_25_WTP$b_Price.post.mean> -5,]
WTP <- PhD_MXL_ModelFive_2022_08_25_WTP %>% select(ends_with(".post.mean")) %>% select(!starts_with("b_Price"))
WTP <- WTP*-1

# ggsave(PhD_MXL_ModelFive_2022_08_25_WTP %>% select(ends_with(".post.mean")) %>% reshape2::melt() %>% ggplot(aes(x=value,y=variable,group=variable,fill=variable))+
#          geom_density_ridges()+geom_vline(xintercept=0,linetype='dashed')+
#          scale_x_continuous(name="mWTP in pounds."), 
#        device = "jpeg",
#        filename = "PhD_MXL_ModelFive_2022_08_25_WTP_DensityPlot.jpeg",
...
ggsave(WTP %>% reshape2::melt() %>% ggplot(aes(x=value,y=variable,group=variable,fill=variable))+
         ...
         scale_x_continuous(name="mWTP in pounds."), 
       device = "jpeg",
       filename = "PhD_MXL_ModelFive_2022_08_25_WTP_NoCost_DensityPlot.jpeg",
       width=20,height=15,units = "cm",dpi=1000)